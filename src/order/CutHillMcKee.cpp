 ...
 *
 *          contains the function to apply CutHillMcKee algorithm to find new
 *          order to reduce fillins
 ******************************************************************************/
void CutHillMcKee(int nedges, int *xadj, int *adjacency, int *order)
{
    // allocate memory
    ...
    for (int i = 0, j = 0, l = 0; i < nedges; i++)
    {
        if (handled[i] == 0)
        {
            int x = i;
